...
class TestGitResetRule:
    ...
    def test_no_match_other_git(
        self, parser: CommandParser, rule: GitResetRule
    ) -> None:
        cmd = parser.parse("git commit -m 'test'")
        assert not rule.matches(cmd)

    def test_git_reset_hard_high_danger(
        self, parser: CommandParser, rule: GitResetRule
    ) -> None:
        cmd = parser.parse("git reset --hard")
    ...
    def test_git_reset_hard_head_critical(
        self, parser: CommandParser, rule: GitResetRule
    ) -> None:
    ...
    def test_git_reset_mixed_medium(
        self, parser: CommandParser, rule: GitResetRule
    ) -> None:
...
class TestGitPushForceRule:
    ...
    def test_matches_push_force(
        self, parser: CommandParser, rule: GitPushForceRule
    ) -> None:
        cmd = parser.parse("git push --force")
        assert rule.matches(cmd)

    def test_matches_push_f(
        self, parser: CommandParser, rule: GitPushForceRule
    ) -> None:
        cmd = parser.parse("git push -f")
        assert rule.matches(cmd)

    def test_no_match_regular_push(
        self, parser: CommandParser, rule: GitPushForceRule
    ) -> None:
        cmd = parser.parse("git push origin main")
        assert not rule.matches(cmd)

    def test_force_push_to_main_critical(
        self, parser: CommandParser, rule: GitPushForceRule
    ) -> None:
    ...
    def test_force_push_with_lease_medium(
        self, parser: CommandParser, rule: GitPushForceRule
    ) -> None:
    ...
    def test_force_push_feature_high(
        self, parser: CommandParser, rule: GitPushForceRule
    ) -> None:
...
class TestGitCleanRule:
    ...
    def test_git_clean_fdx_critical(
        self, parser: CommandParser, rule: GitCleanRule
    ) -> None:
...
class TestGitBranchDeleteRule:
    ...
    def test_matches_branch_D(
        self, parser: CommandParser, rule: GitBranchDeleteRule
    ) -> None:
        cmd = parser.parse("git branch -D feature")
        assert rule.matches(cmd)

    def test_no_match_lowercase_d(
        self, parser: CommandParser, rule: GitBranchDeleteRule
    ) -> None:
        cmd = parser.parse("git branch -d feature")
        assert not rule.matches(cmd)

    def test_force_delete_high(
        self, parser: CommandParser, rule: GitBranchDeleteRule
    ) -> None:
